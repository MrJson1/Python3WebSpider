...
def get_captcha_code(session):
    ...
    url = "http://47.74.208.92:19954/captcha/v1"
    import base64
    def request_1(img_bytes):
        captcha_1 = requests.post(url, json={'image': base64.b64encode(img_bytes).decode()}).json()['message']
        return captcha_1
    captcha = request_1(response.content)
    return captcha
        ...
        try:
            ...
            if response_docs_dict:
                ...
                try:
                    ...
                    dict['District'] = None
                try:
                    ps_name = str(response_docs_dict['ps_name']).strip()
                    AddressInfo = getAddress(ps_name)
                    # print(AddressInfo)
                    dict['LatitudeAndLongitude'] = AddressInfo
                    # Latitude = AddressInfo[0]
                    # Longitude = AddressInfo[1]
                    # print(Latitude)
                    # print(Longitude)
                    dict['PollingStation'] = ps_name
                except:
                    dict['PollingStation'] = None
                    dict['LatitudeAndLongitude'] = None
                try:
...
def getAddress(address):
    ...
    resAddress = requests.post(url=url, headers=headers, data=FormData)
    result = resAddress.text
...
def A(K=1):
    for i in range(1, 100):
        try:
            ...
            print(i)
            if "SJE0997320" in str(result_dict):
                print(9999)
                print("K is##################",K)
                K +=1
