...
def get_post_parameter():
    ...
    try:
        ...
        get_form_rcdl_ = soup.select("div button")[1].attrs['id']
        # print("get_form_rcdl_:", get_form_rcdl_)
        # img_src = soup.select("img#form_rcdl:j_idt37:j_idt43")[0].attrs["src"]
        # print(get_javax_faces_ViewState)
        # print(img_src)
        return get_javax_faces_ViewState,session,get_form_rcdl_
    except:
...
def GetVehicleInfo(soup):
    ...
    try:
        ...
        # print(tempLine)
        Temp = tempLine[0].get_text().strip().replace(' ', '').replace(':', '')
        result_dict['PollutionNorms'] = tempLine[1].get_text().strip()
...
def RunVehicleCheckStatus(RegNO1, RegNO2):
    ...
    """
    # 获取post 请求的参数
    get_javax_faces_ViewState,  session, get_form_rcdl_= get_post_parameter()
    ...
    FormData = {
        ...
        'javax.faces.source': get_form_rcdl_,
        'javax.faces.partial.execute': '@all',
        'javax.faces.partial.render':'form_rcdl:pnl_show form_rcdl:pg_show form_rcdl:rcdl_pnl',
        get_form_rcdl_:get_form_rcdl_,
        'form_rcdl': 'form_rcdl',
...
for i in range(2):
    print("############执行第：", i, "次")
    j = 1
    result_dict = RunVehicleCheckStatus("MH06T","7161")
    # result_dict = RunVehicleCheckStatus("","")
    print(result_dict)

print("RunVehicleCheckStatus time : ", time.time() - t1)
# import time
# t1 = time.time()
# result_dict = RunVehicleCheckStatus("MH06T","7161")
# # result_dict = RunVehicleCheckStatus("","")
